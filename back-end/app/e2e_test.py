...
def test_qcplots_ReturnsCorrectString(socketio_client):
    ...
    assert json.loads(received[1]["args"])["image_list"][0]["alttext"] == "A violin plot displaying the distribution of the n_genes_by_counts observation generated by a QC Plots block"
    assert json.loads(received[1]["args"])["image_list"][1]["image"][:20] == "b'iVBORw0KGgoAAAANSU"
    assert json.loads(received[1]["args"])["image_list"][1]["alttext"] == "A violin plot displaying the distribution of the total_UMIs observation generated by a QC Plots block"
    assert json.loads(received[1]["args"])["image_list"][2]["image"][:20] == "b'iVBORw0KGgoAAAANSU"
    assert json.loads(received[1]["args"])["image_list"][2]["alttext"] == "A violin plot displaying the distribution of the pct_counts_mt observation generated by a QC Plots block"
    assert received[2]["args"] == json.dumps({"end_connection": "end_connection"})
...
def test_variablegenes_ReturnsCorrectString(socketio_client):
    ...
    received = socketio_client.get_received()
    assert len(received) == 3
    assert json.loads(received[1]["args"])["image"][:20] == "b'iVBORw0KGgoAAAANSU"
...
def test_pca_ReturnsCorrectString():
    ...
    assert json.loads(received[2]["args"])["image"][:20] == "b'iVBORw0KGgoAAAANSU"
...
def test_runumap_ReturnsCorrectString():
    ...
    with patch("scanpy.pp.highly_variable_genes"), patch("scanpy.pl.highly_variable_genes"):
        ...
        received = socketio_client.get_received()
    assert len(received) == 5
    assert json.loads(received[3]["args"])["image"][:20] == "b'iVBORw0KGgoAAAANSU"
