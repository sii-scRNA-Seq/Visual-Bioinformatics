...
def test_qcplots_ReturnsCorrectString(socketio_client):
    ...
    received = socketio_client.get_received()
    assert len(received) == 3
    assert len(json.loads(received[1]["args"])["image_list"]) == 1
    assert json.loads(received[1]["args"])["image_list"][0]["title"] == "Sample: 1"
    assert json.loads(received[1]["args"])["image_list"][0]["image"][:20] == "b'iVBORw0KGgoAAAANSU"
    assert json.loads(received[1]["args"])["image_list"][0]["alttext"] == "A violin plot displaying quality control metrics generated by a QC Plots block"
    assert received[2]["args"] == json.dumps({"end_connection": "end_connection"})
